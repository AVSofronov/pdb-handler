...
def counter(atom, borders, distribution):
  ...
  if atom.bfactor == 0:
    ...
    distribution['t', '=0'] += 1
  for b in borders:
    if atom.bfactor > b and atom.bfactor <= (b+10):
...
def elements_asa_distr(soup, borders, mode):
...
#  result.write('\nMethod: '+method)
  ...
  xticks = ['=0']
  for b in borders:
    xticks.append(str(b)+' to '+str(b+10))
...
if mode2 == 'single':
  ...
  elements_asa_distr(asa_soup, borders, mode1)
